...
2. **LoKU IU 3% — SWEEP 5 cách quên** (commit 35b7236). Config D honest (IHL-only) **KHÔNG quên được IU** (Df_AUC 0.951, MIA 0.848): IHL bão hòa trên IU (nhị phân, model tự tin) + forget nhỏ (191, og≈re) → cả val-stop lẫn cossim-stop dừng ở E4. "CHÌA KHÓA" quên (code dòng 699) = `distill_forget_weight` với `teacher='re'` (config D tắt vì honest). → Cell mới **"CELL 4d-SWEEP"** trong `run_kaggle_loku.ipynb`: A/B/E=distill-toward-re (F_re-guided), C=IHL cao(3.0), D=NegGrad(kappa_cls_forget). Chạy 1 session (~2-2.5h, force_redo, mỗi config 1 row distinct id). RUN_IU_LOKU đã =False. **CHỜ user chạy** → gửi log/CSV → CHỌN config quên lành mạnh nhất (Df_AUC↓ về gold, MIA↓, retain giữ, forget_ce≈test_ce). Attach 4 dataset IU. (bug read-only đã fix 20f99c5.)
