# Exp 010 — exp10_image_fila

| Field | Value |
|---|---|
| **Date** | 2026-06-01 |
| **Git commit** | `17cbe55 (dirty)` |
| **Branch** | `master` |
| **Status** | 🔄 Running |
...
Chan doan: MIA chi doc img_logits (eval_unlearning.py per_sample_ce), nhung LoRA+FILA cua exp08 CHI cham BERT (text query/key/value) -> nhanh anh khong nhan tin hieu unlearn -> Forget AUC ket o 0.833 (gold retrained=0.566). exp10 = exp08 + modality-aware PEFT: them FILA subtraction (W*=W-B*A) len cac Conv2d o stage cuoi cua image encoder (img_model.layer7), img_subtract_scale=0.5, van train retain-only. Tat IHL (ihl_forget_weight=0) de CO LAP hieu ung image-FILA. Doi: Forget AUC/F1 giam ve phia retrained, MIA giu/giam, Test AUC/F1 va speed gan nhu khong doi.
...
| Param | Trước | Sau |
|---|---|---|
| `ihl_forget_weight` | `1.5                                            # [EXP 09 KEY] mạnh nhưng IHL self-bounded` | `0.0                                            # [EXP 10] off — isolate image-FILA` |
| `lora_target_modules` | `["query", "key", "value"]                      # BERT attention only — avoids img_model.fc1` | `["query", "key", "value"]                      # BERT (text) attention` |
<!-- /AUTO -->
...
parameters:
  ...
  loku_image_subtract_scale:
    value: 0.5                                            # gentler than text (conv+BN nhạy hơn). Ablate 0.3/0.5/1.0
...
```bash
python training/forgetmi_loku.py --config config.yaml --fresh --exp exp10_image_fila
```
