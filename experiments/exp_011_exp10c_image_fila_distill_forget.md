# Exp 011 — exp10c_image_fila_distill_forget

| Field | Value |
|---|---|
| **Date** | 2026-06-02 |
| **Git commit** | `7164601 (dirty)` |
| **Branch** | `master` |
...
exp10b cho thay scale image-FILA (0.3 vs 0.5) KHONG doi MIA (0.607 vs 0.612) -> scale khong phai can gat. Chan doan: mo conv anh trainable -> fit retain tot (CLS_ret 1.28->0.78) -> loss forget thap (giong member) -> MIA ~0.61. Gold retrained MIA=0 vi forget loss cua no ~ test. exp10c giu image-FILA (scale 0.3) + bat distill_forget_weight=1.0 (nhe) de ep logits forget khop teacher retrained -> keo phan bo forget ve 'chua tung thay' -> MIA giam. Doi: MIA <= ~0.57, Forget-AUC giu thap ~0.80, Test giu cao.
...
| Param | Trước | Sau |
|---|---|---|
| `distill_forget_weight` | `0.0                                            # BỎ - đã chứng minh có hại` | `1.0                                            # [EXP 10c] NHẸ — ép logits forget khớp teacher retrained` |
| `loku_image_subtract_scale` | `0.5                                            # gentler than text (conv+BN nhạy hơn). Ablate 0.3/0.5/1.0` | `0.3                                            # [EXP 10b] 0.5 đẩy forget tốt nhưng MIA tăng 0.562→0.612;` |
<!-- /AUTO -->
...
```bash
python training/forgetmi_loku.py --config config.yaml --fresh --exp exp10c_image_fila_distill_forget
```
