# Exp 016 — exp11c_ihl_sweep_ihl050

| Field | Value |
|---|---|
| **Date** | 2026-06-03 |
| **Git commit** | `0a4b8ed (dirty)` |
| **Branch** | `master` |
...
exp11c sweep IHL de tim sweet-spot (no F_re). IHL=0.5 forget_ce(1.57)<test(1.74) -> con du dia, Forget-AUC 0.758 hoi cao. Tang IHL de Forget-AUC giam ma giu forget_ce <= test_ce (MIA lanh manh). Chon IHL lon nhat con thoa forget_ce ~ test_ce.
...
| Param | Trước | Sau |
|---|---|---|
| `ihl_forget_weight` | `1.5                                            # [EXP 11] on — forget push không cần F_re` | `0.5                                            # [EXP 11b] IHL=1.5 đẩy forget QUÁ tay (forget-loss >> test` |
<!-- /AUTO -->
...
parameters:
  ...
  ihl_forget_weight:
    value: 0.5                                            # [EXP 11b] IHL=1.5 đẩy forget QUÁ tay (forget-loss >> test
                                                          # → MIA 0.143 "giả"/over-forget). Hạ 0.5 để forget chỉ lên ≈ test
                                                          # → MIA về gần retrained tự nhiên. Ablate 0.3/0.5/0.75.
...
```bash
python training/forgetmi_loku.py --config config.yaml --fresh --exp exp11c_ihl_sweep_ihl050
```
...
```
[E00] loss=+7.120  UKR=+0.985  MKR=+2.120  CLS_ret=+1.365  DSL_ret=+0.5112  DSL_frg=+0.0000  IHL_frg=+1.0391  | CosSim(ul,re)=0.6760
[E01] loss=+6.041  UKR=+0.342  MKR=+1.978  CLS_ret=+1.231  DSL_ret=+0.4871  DSL_frg=+0.0000  IHL_frg=+1.0561  | CosSim(ul,re)=0.6038
[E02] loss=+5.273  UKR=+0.303  MKR=+1.849  CLS_ret=+1.069  DSL_ret=+0.3130  DSL_frg=+0.0000  IHL_frg=+1.0274  | CosSim(ul,re)=0.6590
[E03] loss=+4.844  UKR=+0.289  MKR=+1.760  CLS_ret=+0.957  DSL_ret=+0.2513  DSL_frg=+0.0000  IHL_frg=+1.0069  | CosSim(ul,re)=0.6649
[E04] loss=+4.584  UKR=+0.277  MKR=+1.697  CLS_ret=+0.880  DSL_ret=+0.2366  DSL_frg=+0.0000  IHL_frg=+0.9899  | CosSim(ul,re)=0.6775
[E05] loss=+4.461  UKR=+0.272  MKR=+1.658  CLS_ret=+0.826  DSL_ret=+0.2616  DSL_frg=+0.0000  IHL_frg=+0.9750  | CosSim(ul,re)=0.6831
[E06] loss=+4.486  UKR=+0.271  MKR=+1.632  CLS_ret=+0.867  DSL_ret=+0.2489  DSL_frg=+0.0000  IHL_frg=+0.9520  | CosSim(ul,re)=0.6346
[E07] loss=+4.424  UKR=+0.266  MKR=+1.584  CLS_ret=+0.882  DSL_ret=+0.2298  DSL_frg=+0.0000  IHL_frg=+0.9309  | CosSim(ul,re)=0.6618
```
<!-- /AUTO -->
