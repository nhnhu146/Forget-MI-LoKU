...
parser.add_argument('--text_data_dir', # this will need to be changed in the code
        default='/home/shahad.hardan/forgetme/metadata/',
...
parser.add_argument('--img_data_dir', 
        default = '/home/shahad.hardan/physionet.org/files/mimic-cxr/2.1.0/files/',
        help='the input data directory; should contain the .tsv files (or other data files) for the task')
parser.add_argument('--synonyms_dir', # this will need to be changed in the code
        default='/home/shahad.hardan/forgetme/metadata/Synonyms.csv',
        help='the synonyms directory for text noising; should contain the .csv file (or other data file) for the synonyms to be used for noising')
...
parser.add_argument('--bert_pretrained_dir',
        default='/home/shahad.hardan/forgetme/allenai/',  #explore why is it scibert vocab uncase but it still does not work
...
parser.add_argument('--output_dir', # the fine tuned model directory, output of the training data
        default='/l/users/shahad.hardan/output_10per/',
...
parser.add_argument('--forget_set_path', # information about the data split
...
parser.add_argument('--unlearn_batch_size',
        default=4, type=int, help='unlearn batch size')
parser.add_argument('--random_point_ratio',
        default=0.1, type=float, help='ratio of random points generated from training data for the unlearning process')
parser.add_argument('--train_batch_size',
        default=4, type=int, help='train batch size')
parser.add_argument('--eval_batch_size',
        default=4, type=int, help='eval batch size')
parser.add_argument('--learning_rate',
        default=1e-5, type=float, help='Intial learning rate')
parser.add_argument('--num_train_epochs',
        default=300, type=int, help='number of epochs to train for')
parser.add_argument('--grad_accumulation_steps',
        default=10, type=int, help='number of ')
parser.add_argument('--unlearn_epochs',
        default=50, type=int, help='number of epochs to unlearn for')
...
parser.add_argument('--text_noise_level',
        default=0.3, type=float, help='text ratio to be perturbed')
