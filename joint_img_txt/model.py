...
class ImageResNet(nn.Module):
    ...
    def forward(self, x, use_lowerlevel_features=False):
        ...
        # print("Image Embedding:", z[:5])  # Print first 5 image embeddings
        # print("Image Logits:", logits[:5])  # Print first 5 logits
...
class TextBertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
        ...
        embeddings = self.bert.embeddings(input_ids)  # Extract embeddings
        # print("Token Embeddings:", embeddings[:, :5, :])  # Print first 5 tokens for each batch
        # for i, ids in enumerate(input_ids):
        #     print(f"Input {i}: Non-padding Tokens: {ids[attention_mask[i] == 1]}")

        outputs = self.bert(input_ids, position_ids=position_ids, token_type_ids=token_type_ids,
                            attention_mask=attention_mask, head_mask=head_mask)
        embeddings = self.bert.embeddings(input_ids)
        # print("Unique Token Embeddings for Input 0:", embeddings[0, attention_mask[0] == 1, :].unique(dim=0))
        # print("Unique Token Embeddings for Input 1:", embeddings[1, attention_mask[1] == 1, :].unique(dim=0))

        # print("Outputs Keys:", outputs.keys())
        # if outputs.hidden_states is not None:
        #     print(f"Number of Hidden States: {len(outputs.hidden_states)}")
        # else:
        #     print("Hidden States are None")
        # print(f"Hidden State for [CLS] token (Layer 0): {outputs.hidden_states[0][:, 0, :5]}")
        # print(f"Hidden State for [CLS] token (Layer 12): {outputs.hidden_states[12][:, 0, :5]}")
        ...
        # print("Input IDs:", input_ids[:5])  # Check the first 5 input sequences
        # print("Attention Masks:", attention_mask[:5])  # Check the corresponding attention masks

        pooled_output = outputs[1]# this is the default pooled output i.e. [CLS]
        # print("Before Dropout:", pooled_output[:5])  # Output before dropout

        pooled_output = self.dropout(pooled_output)
        # print("After Dropout:", pooled_output[:5])  # Output after dropout

        # if same_classifier:
        #     outputs = (pooled_output,) + outputs[2:]
        # else:
        logits = self.classifier(pooled_output)
        # print("Text CLS Embedding:", pooled_output[:5])  # Print first 5 text embeddings
        # print("Text Logits:", logits[:5])  # Print first 5 logits
...
class ImageTextModel(nn.Module):
    ...
    def forward(self, input_img, input_ids, token_type_ids=None, attention_mask=None, 
        ...
        if bert_pool_use_img and bert_pool_img_lowerlevel:
            img_embedding = lowerlevel_img_feat.to(device)
        elif bert_pool_use_img and not bert_pool_img_lowerlevel:
            img_embedding = z_img.to(device)
        ...
        # print("Image Embedding:", z_img[:5])  # Print first 5 image embeddings
        # print("Image Logits:", logits_img[:5])
        # print("Text Embedding:", z_txt[:5])  # Print first 5 text embeddings
        # print("Text Logits:", logits_txt[:5])
    ...
    # based on 
    # https://github.com/huggingface/transformers/blob/v1.0.0/transformers/modeling_utils.py
